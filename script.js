...
function check_direction() {
	if (total_visible_frames() === 1) { direction = "prev" }
	if (queue.length === 0) { direction = "next" }
}
...
function total_visible_frames() {
	...
	return Array.from(frames).reduce((acc, frame) => {
		if (frame.style.display !== "none") { acc += 1 }
		return acc;
	}, 0)
}


people.forEach((person) => person.images.forEach((img) => create_frame(img, person)))
